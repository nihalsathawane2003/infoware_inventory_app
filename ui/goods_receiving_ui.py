...
class GoodsReceivingUI(QWidget):
    def __init__(self):
        ...
        self.rate_per_unit = QLineEdit()
        self.gst_tax = QLineEdit()
        self.total = QLabel("0.00")

        # Dummy data (replace with DB product list if needed)
        ...
        layout.addRow("Rate per Unit:", self.rate_per_unit)
        layout.addRow("GST %:", self.gst_tax)
    ...
    def submit_goods(self):
        ...
        try:
            ...
            rate = float(self.rate_per_unit.text().strip())
            gst = float(self.gst_tax.text().strip())
        except ValueError:
            QMessageBox.warning(self, "Input Error", "Quantity, Rate, and GST must be numbers.")
            return

        subtotal = quantity * rate
        total_with_gst = subtotal + (subtotal * gst / 100)
